 ...
 *
 * <p>Pattern verified against a reference launcher's per-device
 * classes — each binds a single
 ...
 * {@code register} call needed — that's why the reference doesn't
 ...
 * against the reference's {@code AbsBYDAutoDeviceEx}, which reads
 * the same methods. The (dt) is implied by {@link #getDevicetype()} on
...
public final class BydPushDevice extends AbsBYDAutoDevice
    ...
    public BydPushDevice(Context context, int deviceType, PushSink sink) {
        ...
        // a 10-min observation window). The reference launcher pairs
        // every device with an IBYDAutoListener and explicitly calls
    ...
    /** Wildcard — receive every key for this dt. Matches the
     *  reference AbsBYDAutoDeviceEx pattern, whose getFeatureList
     *  also returns null.
     ...
     * — verified against an older reference build). Without this override:
    ...
    /** IBYDAutoListener push callback. Verified against the reference —
