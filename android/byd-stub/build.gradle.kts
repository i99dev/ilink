 ...
 * the framework ABI byte-for-byte (signatures verified against the
 * smali under
 * `byd/_dudu_extract/dex/smali_dudu/c1/com/dudu/autoui/manage/...`).
 ...
 * stub jar (the way Samsung / Xiaomi do). The smali under
 * `_dudu_extract` is the only source of truth we have for the ABI;
 * these stubs encode it as Java so the Java compiler can type-check
