 ...
 *
 * ABI verified against a reference launcher built for the same framework.
 *
 * <p>Subclassing pattern (per that launcher's per-device subclasses
 * + our {@code BydPushDevice}):
 *
 * <ol>
 *   <li>Override {@link #getDevicetype()} to return a fixed BYD
 *       device-type integer (1000 = AC, 1001 = BODY, 1004 = LIGHT,
 *       …; the full table ships in the bundled dispatch table).
 ...
 *
 * <p>Method signatures here are EXACTLY what the reference launcher
 * calls — adding a method requires confirming its signature against
 * that reference before stubbing it (anything else risks a runtime
...
public abstract class AbsBYDAutoDevice {
    ...
    //
    // Verified against a reference launcher: subclasses call
    // super.registerListener(IBYDAutoListener) after construction.
    //
