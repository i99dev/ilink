...
## What a reference launcher does

A reference launcher for the same hardware uses the pattern we tried:
it calls `AbsBYDAutoDevice.registerListener(IBYDAutoListener)` directly.

It works for them because that launcher installs as a system-signed app
on rooted BYD HUs / custom ROMs. We're a normal-UID Play-Store-style
app; no path to that signing without flashing the device.
